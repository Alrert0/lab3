...
Distributed Computing Lab 3 - Implementation of Raft Consensus Algorithm (Leader Election + Log Replication).
...
--peers: Comma-separated list of other node URLs (without http://localhost prefix for remote peers)
...
Once majority of nodes have the entry, leader commits it
...
Send a command using the client:

bash
python client.py "SET test=hello"
Verify the command replicates across all nodes

Fault Tolerance Test
Start the cluster and send some commands

Kill the leader process

Observe new leader election

Send more commands - they should still be processed

Restart the killed node - it should catch up with the log
